...
SELECT num, data_fabricacao, DATEDIFF(MONTH, GETDATE(), data_fabricacao)
FROM dvd
WHERE id_filme = 1003

SELECT num_dvd, data_locacao, data_locacao, DATEDIFF(DAY, data_devolucao, data_locacao), valor
FROM locacao
WHERE num_cliente = 5505

SELECT nome, logradouro+','+CAST(num AS VARCHAR(5)), (SUBSTRING(cep, 1, 5) + '-' + SUBSTRING(cep, 6, 3))
FROM cliente
WHERE num_cadastro = 5505 & 5503